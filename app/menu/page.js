...
export default function Menu() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setMessage('');
      try {
        const response = await fetch(`${apiUrl}/v1/restaurants/${restaurantId}/menu`, {
  ...
  return <main>
    ...
    <section className="page"><div className="menu-shell"><div>
      {!restaurantId ? <div className="card">Select a restaurant first.</div> : loading ? <div className="card">Loading menu…</div> : message ? <div className="card empty-state"><h3>We couldn’t load this menu.</h3><p className="muted">{message}</p><Link className="primary" href="/restaurants">Back to restaurants</Link></div> : <>
