...
function RestaurantContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const search = new URLSearchParams(); if (query) search.set('q', query); if (cuisine) search.set('cuisine', cuisine); if (filter !== 'all' && filter !== 'veg') search.set('filter', filter);
  ...
  const visible = useMemo(() => filter === 'veg' ? restaurants.filter((restaurant) => `${restaurant.name || ''} ${restaurant.cuisine || ''} ${restaurant.description || ''}`.toLowerCase().includes('veg')) : restaurants, [restaurants, filter]);
