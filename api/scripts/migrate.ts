...
const MIGRATION_LOCK = 'tadka:migrations';

async function main() {
  await pool.query('SELECT pg_advisory_lock(hashtext($1))', [MIGRATION_LOCK]);
  try {
    ...
    await pool.query('SELECT pg_advisory_unlock(hashtext($1))', [MIGRATION_LOCK]);
  }
}
