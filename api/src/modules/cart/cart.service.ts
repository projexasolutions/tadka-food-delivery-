...
export async function getCart(userId: string) {
  ...
  const items = await db.select({ id: cartItems.id, menuItemId: menuItems.id, quantity: cartItems.quantity, name: menuItems.name, description: menuItems.description, price: menuItems.price, imageUrl: menuItems.imageUrl, restaurantId: menuItems.restaurantId, restaurantName: restaurants.name, deliveryFee: restaurants.deliveryFee })
  ...
  const subtotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0); const deliveryFee = items.length ? items[0].deliveryFee : 0;
...
export async function updateCartItem(userId: string, itemId: string, input: UpdateCartItemInput) {
  ...
  const item = await db.select({ id: cartItems.id }).from(cartItems).where(and(eq(cartItems.id, itemId), eq(cartItems.cartId, cart[0].id))).limit(1);
...
export async function removeCartItem(userId: string, itemId: string) { return updateCartItem(userId, itemId, { quantity: 0 }); }
