...
export async function listUsers() { return db.select({ id: users.id, email: users.email, fullName: users.fullName, role: users.role, restaurantId: users.restaurantId, createdAt: users.createdAt }).from(users).orderBy(desc(users.createdAt)); }
...
export async function listRecentOrders() { return db.select({ id: orders.id, restaurantId: orders.restaurantId, restaurantName: restaurants.name, status: orders.status, paymentStatus: orders.paymentStatus, total: orders.total, createdAt: orders.createdAt }).from(orders).innerJoin(restaurants, eq(orders.restaurantId, restaurants.id)).orderBy(desc(orders.createdAt)).limit(25); }
...
export async function updateOrderStatus(orderId: string, input: UpdateOrderStatusInput) {
  ...
  const [updated] = await db.update(orders).set({ status: input.status, updatedAt: new Date() }).where(and(eq(orders.id, orderId), eq(orders.status, order.status))).returning({ id: orders.id, status: orders.status, updatedAt: orders.updatedAt });
  if (!updated) throw new AdminError('Order changed before this update was applied. Refresh and try again.');
  return updated;
}

export async function listCategories() { return db.select({ id: categories.id, restaurantId: categories.restaurantId, restaurantName: restaurants.name, name: categories.name, createdAt: categories.createdAt }).from(categories).leftJoin(restaurants, eq(categories.restaurantId, restaurants.id)).orderBy(desc(categories.createdAt)); }
export async function createCategory(input: CreateCategoryInput) { const [restaurant] = await db.select({ id: restaurants.id }).from(restaurants).where(eq(restaurants.id, input.restaurantId)).limit(1); if (!restaurant) throw new AdminError('Restaurant not found.'); const [category] = await db.insert(categories).values(input).returning({ id: categories.id, restaurantId: categories.restaurantId, name: categories.name }); return category; }
export async function deleteCategory(categoryId: string) { const [deleted] = await db.delete(categories).where(eq(categories.id, categoryId)).returning({ id: categories.id }); if (!deleted) throw new AdminError('Category not found.'); }
