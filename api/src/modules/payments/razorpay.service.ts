...
function requiredEnv(name: string) {
  ...
  if (!value) throw new PaymentError('Payment service is not configured.');
  return value;
}
...
export async function createRazorpayOrder(userId: string, input: CreateRazorpayOrderInput) {
  ...
  if (!response.ok || !body?.id) throw new PaymentError('Unable to create the payment order.');
...
export async function verifyRazorpayPayment(userId: string, input: VerifyRazorpayPaymentInput) {
  const [order] = await db.select({ id: orders.id, razorpayOrderId: orders.razorpayOrderId, paymentStatus: orders.paymentStatus })
    .from(orders).where(and(eq(orders.id, input.orderId), eq(orders.userId, userId))).limit(1);
  if (!order) throw new PaymentError('Order not found.');
  ...
  const supplied = Buffer.from(input.razorpaySignature, 'utf8');
