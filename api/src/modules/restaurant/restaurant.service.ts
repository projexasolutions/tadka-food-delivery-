...
import { categories, menuItems, orders, orderItems, restaurants, users } from '../../db/schema';
...
async function getStaffRestaurantId(userId: string) {
  const [user] = await db.select({ restaurantId: users.restaurantId })
...
export async function listStaffMenu(userId: string) {
  ...
  const [restaurant, items, categoryRows] = await Promise.all([
    db.select({ id: restaurants.id, name: restaurants.name, isOpen: restaurants.isOpen })
      .from(restaurants).where(eq(restaurants.id, restaurantId)).limit(1),
    db.select({ id: menuItems.id, name: menuItems.name, description: menuItems.description, price: menuItems.price,
      imageUrl: menuItems.imageUrl, categoryId: menuItems.categoryId, isAvailable: menuItems.isAvailable })
      .from(menuItems).where(eq(menuItems.restaurantId, restaurantId)).orderBy(asc(menuItems.createdAt)),
    db.select({ id: categories.id, name: categories.name }).from(categories)
      .where(eq(categories.restaurantId, restaurantId)).orderBy(asc(categories.name)),
  ]);
...
export async function createMenuItem(userId: string, input: CreateMenuItemInput) {
  ...
  const [item] = await db.insert(menuItems).values({
    restaurantId, name: input.name, description: input.description ?? null, price: input.price,
    categoryId: input.categoryId ?? null, imageUrl: input.imageUrl ?? null, isAvailable: input.isAvailable ?? true,
  }).returning();
  return item;
}
...
export async function updateMenuItem(userId: string, menuItemId: string, input: UpdateMenuItemInput) {
  ...
  const [existing] = await db.select({ id: menuItems.id }).from(menuItems)
    .where(and(eq(menuItems.id, menuItemId), eq(menuItems.restaurantId, restaurantId))).limit(1);
  if (!existing) throw new RestaurantOperationError('Menu item not found.');
  if (input.categoryId) await assertCategoryBelongsToRestaurant(input.categoryId, restaurantId);
  const [item] = await db.update(menuItems).set({ ...input, updatedAt: undefined }).where(eq(menuItems.id, menuItemId)).returning();
  return item;
}
...
export async function setMenuItemAvailability(userId: string, menuItemId: string, isAvailable: boolean) {
  ...
  const [item] = await db.update(menuItems).set({ isAvailable })
    .where(and(eq(menuItems.id, menuItemId), eq(menuItems.restaurantId, restaurantId))).returning();
...
export async function updateCategory(userId: string, categoryId: string, name: string) {
  const restaurantId = await getStaffRestaurantId(userId);
  const [category] = await db.update(categories).set({ name })
    .where(and(eq(categories.id, categoryId), eq(categories.restaurantId, restaurantId))).returning();
...
export async function deleteCategory(userId: string, categoryId: string) {
  ...
  const [category] = await db.delete(categories)
    .where(and(eq(categories.id, categoryId), eq(categories.restaurantId, restaurantId))).returning({ id: categories.id });
  if (!category) throw new RestaurantOperationError('Category not found.');
}
...
export async function listStaffOrders(userId: string) {
  ...
  const rows = await db.select({ id: orders.id, status: orders.status, paymentMethod: orders.paymentMethod,
    paymentStatus: orders.paymentStatus, deliveryAddress: orders.deliveryAddress, phone: orders.phone,
    total: orders.total, createdAt: orders.createdAt, customerName: users.fullName, customerEmail: users.email })
    .from(orders).innerJoin(users, eq(orders.userId, users.id))
    .where(eq(orders.restaurantId, restaurantId)).orderBy(desc(orders.createdAt)).limit(50);
  return rows;
}
...
export async function updateStaffOrderStatus(userId: string, orderId: string, nextStatus: string) {
  ...
  const allowed: Record<string, string[]> = {
    pending: ['confirmed', 'cancelled'], confirmed: ['preparing', 'cancelled'], preparing: ['ready'], ready: [],
    picked_up: [], delivered: [], cancelled: [],
  };
  if (!allowed[order.status]?.includes(nextStatus)) {
    throw new RestaurantOperationError(`Order cannot move from ${order.status} to ${nextStatus}.`);
  }
  const [updated] = await db.update(orders).set({ status: nextStatus, updatedAt: new Date() })
    .where(eq(orders.id, orderId)).returning({ id: orders.id, status: orders.status, updatedAt: orders.updatedAt });
  return updated;
}
