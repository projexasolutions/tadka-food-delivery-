...
export async function createReview(userId: string, orderId: string, input: CreateReviewInput) {
  return db.transaction(async (tx) => {
    const [order] = await tx.select({ id: orders.id, restaurantId: orders.restaurantId, status: orders.status, userId: orders.userId }).from(orders).where(and(eq(orders.id, orderId), eq(orders.userId, userId))).limit(1);
...
export async function listRestaurantReviews(restaurantId: string) {
  return db.select({ id: reviews.id, orderId: reviews.orderId, rating: reviews.rating, comment: reviews.comment, createdAt: reviews.createdAt, customerName: restaurants.name })
    .from(reviews).innerJoin(restaurants, eq(reviews.restaurantId, restaurants.id)).where(eq(reviews.restaurantId, restaurantId)).orderBy(desc(reviews.createdAt));
}
